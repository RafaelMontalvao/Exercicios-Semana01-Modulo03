...
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.status;
    ...
    class VeiculosControllerTest {
        ...
        void incluir_jaCadastrado() throws Exception {
            Mockito.when(service.inserir(Mockito.any(Veiculos.class))).thenThrow(RegistroExistenteException.class);
        ...
        void incluir_sucesso() throws Exception {
            ...
            Veiculos veiculo = modelMapper.map(req, Veiculos.class);
            String requestJson = objectMapper.writeValueAsString(req);
            Mockito.when(service.inserir(Mockito.any(Veiculos.class))).thenReturn(veiculo);
            mockMvc.perform(post("/api/veiculos")
                            .content(requestJson)
                            .contentType(MediaType.APPLICATION_JSON))
        ...
        void consultar() throws Exception {
            var registros = List.of(
                    new Veiculos("ABC-123", "carro", "azul", 2020),
                    new Veiculos("ZXC-456", "caminhao", "prata", 2022)
            );
        ...
        void consultarPorPlaca() throws Exception {
            var veiculo = new Veiculos("ABC-123", "carro", "azul", 2020);
